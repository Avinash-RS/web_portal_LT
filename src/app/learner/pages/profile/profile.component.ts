...
// View Profile
  ...
  removelastQual(index){
    if(this.qual.length == 1){
      this.alert.openAlert("Can't delete the row when there is only one row");  
      return false;  
    }else{
  ...
  removenewLink(index){
    if(this.links.length == 1){
      this.alert.openAlert("Can't delete  when there is only one row");  
      return false;  
    }else{
  ...
  onSelectFile(event) {
     ...
     if(this.selectfile.type != 'image/png'){
       this.alert.openAlert('mage should be less than 1 MB and should be only Jpeg or png format')
     } 
     else if (this.selectfile.size > 1000){
      this.alert.openAlert('image should be less than 1 MB and should be only Jpeg or png format')
     } 
     else {
